...
public class SandboxArreglos
{
    ...
    {
    	 int[] result = Arrays.stream(arregloEnteros).filter(x -> x != valor).toArray();
    ...
    {
    	String[] result = Arrays.stream(arregloCadenas).filter(x -> !x.equals(cadena)).toArray(String[]::new);
    ...
    public void eliminarEnteroPorPosicion( int posicion )
    {
    	if (posicion >= 0 && posicion < arregloEnteros.length) {
            ...
            for (int i = 0; i < arregloEnteros.length; i++) {
                if (i != posicion) {
                    newArray[index] = arregloEnteros[i];
                    index++;
                }
            }
            arregloEnteros = newArray;
